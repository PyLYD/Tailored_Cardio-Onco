...
print(input_dim)   # check Number of features, in this case: 4507
...
def custom_r2(X, X_reconstr):
    ...
    #return 1 - np.sum((X - X_reconstr) ** 2) / np.sum((X - X.mean()) ** 2)
...
def save_encoded_data(encoded_data, latent_dim):
    file_path = f'C:/Users/lyaziddy/Desktop/XAIM_PAVIA/AE/encoded_data_latent_{latent_dim}.npy'
    np.save(file_path, encoded_data)
...
for latent_dim in latent_dims:
    ...
    # Save the models and training history    #Alessandro suggested to save only the weights. We only need to make prediction later on on val data
    #autoencoder.save(f'C:/Users/lyaziddy/Desktop/XAIM_PAVIA/AE/autoencoder_DEA_model_updated_{latent_dim}.keras')
    #encoder.save(f'C:/Users/lyaziddy/Desktop/XAIM_PAVIA/AE/encoder_DEA_model_updated_{latent_dim}.keras')
    with open(f'C:/Users/lyaziddy/Desktop/XAIM_PAVIA/AE/AE_training_history_updated_{latent_dim}.pkl', 'wb') as file:
        pickle.dump(history.history, file)
    autoencoder.save_weights(f'C:/Users/lyaziddy/Desktop/XAIM_PAVIA/AE/model_weights_{latent_dim}.weights.h5')

    # Save encoded data
    encoded_data = encoder.predict(x_train)
    save_encoded_data(encoded_data, latent_dim)

    # encoded_data_val = encoder.predict(x_val)
    # save_encoded_data(encoded_data_val, latent_dim)
